/**
 * The main file to run the DTN protocol
 */
...
void * oldMain() {
	...
	//Set up to catch timer signal to send beacon on a regular intrval
...
int initialize(config * c) {
	...
	//Set up the main socket used to send/receive all packets
	//This socket receives from any interface and is capable of broadcasts
	...
	//Create thread that handles beacons and receiving packets
	pthread_t run;
	...
	//Configure gateway mode if necessary
...
void send_beacon() {
	...
	//Send the beacon packet to all beacon addresses
	char buf[20];
	int i = 0, j = 0;
...
void data_handler(packet * p) {
	...
	//Determine if this packet has or has not been ACKed before
	if(isOld(p)==NOT_OLD_PACKET) {
	
		takePacket = 0;
		//If in gateway mode determine if this packet is destined outside the bundle network
		if (gateway) {
			char pNet[4];
		...
		//The received packet is destined for this node
		if(!memcmp(p->dest, configuration->IP, 4) || takePacket==1) {	
			//Deliver the packet to the data queue, which may return an ACK
			ack=deliverPacket(p);
			if(ack!=NULL) {
				isOld(ack);
				add_packetnode(ack);
...
void ack_handler(packet * p) {
	if(isOld(p)==NOT_OLD_PACKET) {
		//Add to the forwarding queue
		add_packetnode(p);
		//Delete all old packets
        delete_packetnode(p);
	}
	return;
}
